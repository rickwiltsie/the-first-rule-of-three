/*
This adds drag and drop functionality through Alpine JS. Install with Alpine.data()

Config...
onDropCallback: callback function for when a dragWrapper is dropped on a valid target.

Sample usage...
<div x-data="dragDrop({
    onDropCallback: (dragEl, endDragEl, containerEl, hoverTopHalf) => {
        console.log(dragEl);
        console.log(endDragEl);
    }
})">
    <div x-bind="dropContainer()">
        <div x-bind="dragWrapper()">
            <div x-bind="dragHandle()">
                Heading
            </div>
            <p>Content</p>
        </div>
        <div x-bind="dragWrapper()"></div>
    </div>
    <div x-bind="dropContainer()">
        <div x-bind="dragWrapper()"></div>
        <div x-bind="dragWrapper()"></div>
